...
%     min_DFdif = inf;
%     min_ind = 1;
%     for iter=1:DF_maxIter
%         DF = relativeDynamicsDistance_HHp(tempXHHp{i}, tempXHHpFrob{i}, ind, w);
%         [~, ind] = getCenter(tempX{i}, DF);
%         DFdif = abs(DF - mean(DF));
%         if DFdif < min_DFdif
%             min_DFdif = DFdif;
%             min_ind = ind;
%         end
...
%     end
%     ind = min_ind;
%     centers(:,i) = tempX{i}(:, ind);
%     indAbsolute(i) = indicatorI{i}(ind);
    % modified by xikang
    DFset = zeros(1,size(tempXHHp{i},2));
    for j=1:size(tempXHHp{i},2)
        DF = relativeDynamicsDistance_HHp(tempXHHp{i},tempXHHpFrob{i},j,w);
        DFset(j) = sum(DF);
    end
    [~,ind] = max(DFset);
    centers(:,i) = tempX{i}(:, ind);
